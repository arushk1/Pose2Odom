...
OdometryGenerator::OdometryGenerator(ros::NodeHandle &n)
{
    ...
    sub = n.subscribe("/optitrack/rgbd_sensor/pose", 1, &OdometryGenerator::MocapDataCallback, this);
}
...
void OdometryGenerator::MocapDataCallback(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    ...
    current_odom_pose.position.x = msg->pose.position.x - init_pose.position.x;
    current_odom_pose.position.y = msg->pose.position.y - init_pose.position.y;
    ...
    Eigen::Quaternion<double> current_quat = init_quat.inverse() * msg_quat; // TODO: not sure about this operation
    current_odom_pose.orientation.x = current_quat.x();
    current_odom_pose.orientation.y = current_quat.y();
    current_odom_pose.orientation.z = current_quat.z();
    current_odom_pose.orientation.w = current_quat.w();
    ...
    drone2map_tf_msg.child_frame_id  = "rgbd_sensor/odom/base_link";
    ...
    odom.child_frame_id  = "rgbd_sensor/odom/base_link";
    ...
    try{
        listener.lookupTransform("rgbd_sensor/odom/base_link", "map",
                                 ros::Time(0), drone2map_tf);
        listener.lookupTransform("world", "rgbd_sensor/base_link",
